...
{
    // (text, bubbleType, durationMs, title, actions)
    public event Action<string, string, int, string?, IReadOnlyList<StockReview.Core.Services.BubbleAction>?>? BubbleRequested;
    ...
    public void ShowBubble(string text, string type = "encourage", int durationMs = 8000, string? title = null,
        ...
        => BubbleRequested?.Invoke(text, type, durationMs, title, actions);
