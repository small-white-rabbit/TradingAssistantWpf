...
│   │   ├── DatabaseService.cs       # 数据库服务（对应 sqlite-layer.cjs）
...
直接沿用 Electron 版的 `data/data.db` 文件，schema 零改动。
WAL 模式保持不变，两套程序可共用同一数据库（避免同时写）。
...
- [x] planScheduler 业务逻辑翻译（`Services/PlanSchedulerService.cs`，4620 行；对照基准 `src/stores/planScheduler.js`）
- [x] sellPointDetector 业务逻辑翻译（`Engines/SellPointDetectorService.cs`，3882 行；对照基准 `src/stores/sellPointDetector.js`）
- [x] buyPointDetector 业务逻辑翻译（`Engines/BuyPointDetectorService.cs`，851 行；对照基准 `src/stores/buyPointDetector.js`）
